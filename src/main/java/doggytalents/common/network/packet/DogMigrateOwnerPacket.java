	...
	public void handleDog(Dog dog, DogMigrateOwnerData data, Supplier<Context> ctx) {
		...
		if (!data.confirmed) {
			requester.sendSystemMessage(Component
					.translatable("item.doggytalents.amnesia_bone.migrate_owner.reject", dog.getName().getString())
		...
		// Not enough level
		if (sender.experienceLevel < AmnesiaBoneItem.MigrateOwnerXPCost)
			return;
		...
		// Proccess sender
		sender.giveExperienceLevels(-AmnesiaBoneItem.MigrateOwnerXPCost);
		...
		// Proccess requester
		requester.sendSystemMessage(Component.translatable("item.doggytalents.amnesia_bone.migrate_owner.confirmed",
