...
//        generated(DoggyBlocks.RICE_MILL);
	...
	private ItemModelBuilder radar(Supplier<? extends ItemLike> item, ResourceLocation texture) {
		...
		builder.transforms().transform(ItemDisplayContext.THIRD_PERSON_RIGHT_HAND).rotation(0, 0, 55F)
				.translation(0, 4F, 0.5F).scale(0.85F);
		builder.transforms().transform(ItemDisplayContext.THIRD_PERSON_LEFT_HAND).rotation(0, 0, -55F)
				.translation(0, 4F, 0.5F).scale(0.85F);
		builder.transforms().transform(ItemDisplayContext.FIRST_PERSON_RIGHT_HAND).translation(-3.13F, 3.2F, 1.13F)
				.scale(0.8F);
		builder.transforms().transform(ItemDisplayContext.FIRST_PERSON_LEFT_HAND).translation(-3.13F, 3.2F, 1.13F)
				.scale(0.8F);
		return builder;
	}
	...
	private ItemModelBuilder wetFrisbee(Supplier<? extends ItemLike> item) {
		return generated2(item, modLoc(ModelProvider.ITEM_FOLDER + "/frisbee"),
				modLoc(ModelProvider.ITEM_FOLDER + "/frisbee_overlay"));
	}

	private ItemModelBuilder birthdayHat(Supplier<? extends ItemLike> item) {
		return generated2(item, modLoc(ModelProvider.ITEM_FOLDER + "/birthday_hat_bg"),
				modLoc(ModelProvider.ITEM_FOLDER + "/birthday_hat_fg"));
	}

	private ItemModelBuilder ceremonialGarb(Supplier<? extends ItemLike> item) {
		return generated2(item, modLoc(ModelProvider.ITEM_FOLDER + "/" + name(item)),
				modLoc(ModelProvider.ITEM_FOLDER + "/" + name(item) + "_overlay"));
	}
	...
	private ItemModelBuilder scentTreatDrooled(Supplier<? extends ItemLike> item, Supplier<? extends ItemLike> base) {
		return generated2(item, modLoc(ModelProvider.ITEM_FOLDER + "/" + name(base)),
				modLoc(ModelProvider.ITEM_FOLDER + "/" + name(base) + "_overlay"));
	}
	...
	private ItemModelBuilder generated2(Supplier<? extends ItemLike> item, ResourceLocation tex0,
			ResourceLocation tex1) {
		return getBuilder(name(item)).parent(new UncheckedModelFile(ModelProvider.ITEM_FOLDER + "/generated"))
				.texture("layer0", tex0).texture("layer1", tex1);
	}
	...
	private ItemModelBuilder handheld(Supplier<? extends ItemLike> item, ResourceLocation texture) {
		return getBuilder(name(item)).parent(new UncheckedModelFile(ModelProvider.ITEM_FOLDER + "/handheld"))
				.texture("layer0", texture);
	}
