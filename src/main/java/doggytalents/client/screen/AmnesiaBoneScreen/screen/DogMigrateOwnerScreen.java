...
public class DogMigrateOwnerScreen extends Screen {
	...
	protected void init() {
		...
		uuidShowButton = new TextOnlyButton(0, 0, 100, 20,
				Component.translatable("doggui.migrate_owner.show_uuid").withStyle(ChatFormatting.GRAY), $ -> {
			...
			public void render(GuiGraphics graphics, int mouseX, int mouseY, float pTicks) {
				...
				graphics.renderComponentTooltip(font,
						List.of(Component.literal(migrateTo == null ? "UUID_ZERO" : migrateTo.toString())), mouseX,
						mouseY);
			}
		};
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float pTicks) {
		...
		graphics.drawString(font, title, Mth.floor(mX / 1.2f - font.width(title) / 2), Mth.floor(pY / 1.2f),
				0xffffffff);
		stack.popPose();
		pY += 40;
		for (var line : help) {
	...
	public void drawWhenHaveRequest(GuiGraphics graphics, int mouseX, int mouseY, float pTicks, UUID newOwnerUUID,
			String newOwnerName) {
		...
		title = Component.translatable("doggui.migrate_owner.confirm.title")
				.withStyle(Style.EMPTY.withBold(true).withColor(ChatFormatting.RED));
		help = I18n.get("doggui.migrate_owner.confirm.subtitle", dog.getName().getString());
		var dog_title = I18n.get("doggui.migrate_owner.info.dog", dog.getName().getString());
		var owner_title = Component.translatable("doggui.migrate_owner.info.new_owner", newOwnerName);
		var costStr = I18n.get("doggui.talents.cost") + AmnesiaBoneItem.MigrateOwnerXPCost;
		var escToReturn = I18n.get("doggui.invalid_dog.esc_to_return");
		stack.pushPose();
		stack.scale(1.2f, 1.2f, 1.2f);
		graphics.drawString(font, title, Mth.floor(mX / 1.2f - font.width(title) / 2), Mth.floor(pY / 1.2f),
	...
	private void addConfirmButton() {
		var confirmButton = new CustomButton(this.width / 2 + 2, this.height / 2 + 58, 50, 20,
				Component.translatable("doggui.migrate_owner.confirm"), b -> {
			...
			public void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float pTicks) {
				...
				this.active = (player != null && player.experienceLevel >= AmnesiaBoneItem.MigrateOwnerXPCost);
			}
			...
			public void render(GuiGraphics graphics, int mouseX, int mouseY, float pTicks) {
				...
				if (this.active) {
					...
					var player = Minecraft.getInstance().player;
					if (player != null && player.experienceLevel < AmnesiaBoneItem.MigrateOwnerXPCost) {
						c1 = Component.translatable("doggui.detrain.talents.insufficent_xp");
						c1.setStyle(Style.EMPTY.withColor(0xffB20000).withBold(true));
					} else {
						return;
					}
				}
		...
		var player = Minecraft.getInstance().player;
		confirmButton.active = (player != null && player.experienceLevel >= AmnesiaBoneItem.MigrateOwnerXPCost);

		this.addRenderableWidget(confirmButton);
	}

	public void addDenyButton() {
		var denyButton = new CustomButton(this.width / 2 - 50 - 2, this.height / 2 + 58, 50, 20,
				Component.translatable("doggui.migrate_owner.deny"), b -> {
	...
	private void requestMigrateOwner(boolean confirmed) {
		PacketHandler.send(PacketDistributor.SERVER.noArg(), new DogMigrateOwnerData(dog.getId(), confirmed));
	}
